    ...
    @if (auth()->user()->isAdmin())
        <aside class="hidden min-h-screen w-64 shrink-0 border-r border-gray-800 bg-card lg:flex lg:flex-col">
            ...
            <nav class="flex-1 space-y-1 px-3 py-4 text-sm">
                ...
                <a href="{{ route('admin.verifications.index') }}" class="block rounded-lg px-3 py-2 font-medium {{ request()->routeIs('admin.verifications.*') ? 'bg-primary text-white' : 'text-gray-300 hover:bg-primary hover:text-white' }}">
                    Verifications
                </a>
            </nav>
        </aside>
