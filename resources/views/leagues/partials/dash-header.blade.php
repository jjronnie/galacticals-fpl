...
<div class="w-full space-y-4" x-data="{ 
    ...
    <!-- Status Messages -->
    @if(session('status'))
    <div class="bg-gradient-to-r from-green-900/40 to-green-800/20 border border-green-700/50 rounded-xl shadow-lg p-4 backdrop-blur-sm">
    ...
    @if($errors->any())
    <div class="bg-gradient-to-r from-red-900/40 to-red-800/20 border border-red-700/50 rounded-xl shadow-lg p-4 backdrop-blur-sm">
        <div class="flex items-start gap-3">
            <div class="flex-shrink-0 mt-0.5">
    ...
    <div class="bg-gradient-to-r from-blue-900/40 to-blue-800/20 border border-blue-700/50 rounded-xl shadow-lg p-4 backdrop-blur-sm">
        <div class="flex items-start gap-3">
            <div class="flex-shrink-0 mt-0.5">
                ...
                <div class="space-y-2">
                    ...
                    <div class="flex justify-between items-center text-xs">
                        ...
                        <span class="text-blue-500 font-bold" x-text="`${syncProgress}%`">
                            {{ $league->total_managers > 0 ? round(($league->synced_managers / $league->total_managers) * 100) : 0 }}%
                        </span>
                    </div>
                </div>
            </div>
        ...
        <div class="flex items-start gap-3">
            ...
            <div class="flex-1">
                ...
                <form action="{{ route('league.update') }}" method="POST" class="inline">
                    @csrf
                    <button type="submit" class="text-sm font-medium text-red-400 hover:text-red-300 transition-colors inline-flex items-center gap-2">
    ...
    <div class="bg-card rounded-xl shadow-lg border border-gray-700/50 overflow-hidden relative">
        ...
        <div class="absolute top-0 right-0 -mt-4 -mr-4 w-32 h-32 bg-primary/10 rounded-full blur-3xl pointer-events-none"></div>
            ...
            <div class="flex items-center gap-3 w-full sm:w-auto justify-start sm:justify-end">
                ...
                <div class="relative">
                    ...
                    <div x-show="copied" x-transition:enter="transition ease-out duration-300"
                        ...
                        Link Copied!
                        <div class="absolute top-full left-1/2 transform -translate-x-1/2 border-4 border-transparent border-t-green-500"></div>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div x-cloak x-show="updating && syncStatus !== 'processing'" 
        ...
        <div class="bg-gray-900 border border-gray-700 text-white p-6 rounded-xl shadow-2xl flex flex-col items-center gap-4 max-w-sm mx-4">
