...
class FetchSingleManagerProfileJob implements ShouldQueue
{
    ...
    public function __construct(
        ...
        private readonly array $managerIds,
    ) {}

    public function handle(): void
    {
        Log::info('Syncing single manager profile entry.', [
        ...
        $managers->each(fn (Manager $manager) => $manager->update([
            'sync_status' => 'processing',
            'sync_message' => 'Syncing profile data...',
        ]));

        try {
            $profilePayload = $this->fetchJson("entry/{$this->entryId}/");
            $historyPayload = $this->fetchJson("entry/{$this->entryId}/history/");

            $this->syncManagerIdentity($managers, $profilePayload);
            $this->syncGameweekHistory($managers, $historyPayload['current'] ?? []);
            $this->syncChips($managers, $historyPayload['chips'] ?? []);

            $gameweeksToSync = $this->resolveGameweeksToSync($managers);
            $failedGameweeks = [];

            foreach ($gameweeksToSync as $gameweek) {
                try {
                    $picksPayload = $this->fetchJson("entry/{$this->entryId}/event/{$gameweek}/picks/");
                    $pointsMap = $this->cachedEventPointsForGameweek($gameweek);
                    ...
                    Log::warning('Failed to sync picks for gameweek; continuing.', [
                        'entry_id' => $this->entryId,
                        'manager_ids' => $this->managerIds,
                        'gameweek' => $gameweek,
            ...
            $syncMessage = $failedGameweeks === []
                ...
                : $this->truncateMessage(
                    sprintf('Profile synced with gaps in GW %s', implode(', ', array_slice(array_unique($failedGameweeks), 0, 6))),
                    255
                );

            Manager::whereIn('id', $this->managerIds)->update([
                'sync_status' => 'completed',
                'sync_message' => $syncMessage,
                'last_synced_at' => now(),
            ]);

            $this->forgetProfileCaches();
            $this->forgetLeagueAndDashboardCaches($managers, $gameweeksToSync);

            Log::info('Single manager profile entry synced.', [
                'entry_id' => $this->entryId,
                'manager_ids' => $this->managerIds,
                'gameweeks_synced' => count($gameweeksToSync),
                'failed_gameweeks' => $failedGameweeks,
            ]);
        } catch (\Throwable $exception) {
            Log::warning('Failed to sync manager profile entry.', [
                'entry_id' => $this->entryId,
                'manager_ids' => $this->managerIds,
                'exception_class' => $exception::class,
                'error' => $exception->getMessage(),
            ...
            Manager::whereIn('id', $this->managerIds)->update([
                'sync_status' => 'failed',
                'sync_message' => $this->truncateMessage('Failed: '.$exception->getMessage(), 255),
            ]);
        }
    ...
    private function managerQuery()
    {
        return Manager::query()
            ->with('league:id,season')
            ->whereIn('id', $this->managerIds);
    }

    /**
     * Resolve which gameweeks need pick data fetched for this entry.
     *
     * Strategy:
     * - Always sync the current gameweek
     * - Always sync the last finished gameweek (fresh data)
     * - Sync any gameweek that is missing pick data
     *
     * This avoids re-fetching 35+ historical gameweeks that never change.
     *
     * @param  Collection<int, Manager>  $managers
     * @return array<int>
     */
    private function resolveGameweeksToSync(Collection $managers): array
    {
        ...
        $allGameweeks = collect($bootstrapData['events'] ?? [])
        ...
        $lastFinished = $finishedGameweeks[0] ?? null;
        $currentGameweek = collect($bootstrapData['events'] ?? [])
            ->first(fn (array $event): bool => (bool) ($event['is_current'] ?? false));
        $currentGameweekId = $currentGameweek !== null ? (int) $currentGameweek['id'] : null;

        return collect($allGameweeks)
            ->filter(function (int $gameweek) use ($existingPickCounts, $expectedPickRows, $lastFinished, $currentGameweekId): bool {
                if ($gameweek === $currentGameweekId) {
                    return true;
                }

                if ($gameweek === $lastFinished) {
                    return true;
                }

                $pickCount = (int) ($existingPickCounts[$gameweek] ?? 0);

                return $pickCount < $expectedPickRows;
            })
            ->sort()
            ->values()
            ->all();
     ...
     */
    private function cachedEventPointsForGameweek(int $gameweek): array
    {
        ...
        if (is_array($cached)) {
            $this->eventPointsCache[$gameweek] = $cached;
    ...
    private function syncPicksForManager(Manager $manager, int $gameweek, array $payload, array $pointsMap): void
    {
        ...
        $autopSubPoints = collect($payload['automatic_subs'] ?? [])->sum(function (array $sub) use ($pointsMap): int {
            return (int) (($pointsMap[(int) ($sub['element_in'] ?? 0)] ?? 0) - ($pointsMap[(int) ($sub['element_out'] ?? 0)] ?? 0));
        });
    ...
    private function syncManagerIdentity(Collection $entryManagers, array $profilePayload): void
    {
        ...
        if ($favouriteTeamKey !== null) {
            $rawValue = $profilePayload[$favouriteTeamKey];
            $resolvedId = $this->resolveFavouriteTeamId($rawValue);

            if ($resolvedId !== null) {
                $attributes['favourite_team_id'] = $resolvedId;
            } else {
                Log::warning('Could not resolve favourite team for manager.', [
                    'manager_ids' => $managerIds,
                    'payload_key' => $favouriteTeamKey,
                    'raw_value' => $rawValue,
                    'raw_type' => get_debug_type($rawValue),
                ]);
            }
        } else {
            Log::debug('No favourite team key found in FPL profile payload.', [
                'manager_ids' => $managerIds,
                'available_keys' => array_keys($profilePayload),
            ]);
        }
    ...
    private function forgetProfileCaches(): void
    {
        foreach ($this->managerIds as $managerId) {
            Cache::forget('profile_stats_'.$managerId);
        }

        Cache::forget('profile_stats_entry_'.$this->entryId);
        ...
        foreach ([
            ...
            Cache::forget("profile_stats_entry_{$this->entryId}_{$section}");
            Cache::forget("profile_stats_entry_v3_{$this->entryId}_{$section}");
        }
    }
    ...
    {
        Log::error('FetchSingleManagerProfileJob failed permanently.', [
            'entry_id' => $this->entryId,
            'manager_ids' => $this->managerIds,
            'error' => $exception->getMessage(),
        ]);
        ...
        Manager::whereIn('id', $this->managerIds)->update([
            'sync_status' => 'failed',
            'sync_message' => $this->truncateMessage('Sync failed after retries: '.$exception->getMessage(), 255),
        ]);
